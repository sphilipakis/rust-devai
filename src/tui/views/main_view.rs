...
impl MainView {
	fn render_nav(&self, area: Rect, buf: &mut Buffer, state: &mut MainState) {
		Block::new().bg(CLR_BKG_GRAY_DARKER).render(area, buf);

		let runs = RunBmc::list(&self.mm, None).unwrap_or_default();
	...
	fn render_content(&self, area: Rect, buf: &mut Buffer) {
		...
		match LogBmc::list(&self.mm, None) {
			Ok(logs) => {
